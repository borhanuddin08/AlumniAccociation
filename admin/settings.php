...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_department'])) {
    $deptName = sanitizeInput($_POST['department_name'] ?? '');
    ...
    if (!empty($deptName)) {
        try {
            ...
            if ($stmt->execute()) {
                redirectWithMessage(SITE_URL . '/admin/settings.php', 'Department added successfully!', 'success');
            }
        } catch (mysqli_sql_exception $e) {
            if ($e->getCode() == 1062) { // Duplicate entry
                redirectWithMessage(SITE_URL . '/admin/settings.php', 'Department already exists.', 'error');
            } else {
                redirectWithMessage(SITE_URL . '/admin/settings.php', 'Error adding department.', 'error');
            }
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_department'])) {
    ...
    if ($stmt->execute()) {
        redirectWithMessage(SITE_URL . '/admin/settings.php', 'Department deleted successfully!', 'success');
    }
}
            ...
            <div class="card mb-4">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <button type="button" class="btn btn-primary btn-sm" data-mdb-toggle="modal" data-mdb-target="#addDeptModal">
...
<div class="modal fade" id="addDeptModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">
                    <i class="fas fa-plus me-2 text-primary"></i>Add New Department
                </h5>
                <button type="button" class="btn-close" data-mdb-dismiss="modal"></button>
            </div>
            <form method="POST">
